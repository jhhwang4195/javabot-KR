...
public class SearchCmd extends MusicCommand 
{
    ...
    private class ResultHandler implements AudioLoadResultHandler 
    {
        ...
        public void trackLoaded(AudioTrack track)
        {
            ...
            {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" 이 재생목록은 (**"+track.getInfo().title+"**) 허용 된 최대 값보다 깁니다.: `"
        ...
        public void playlistLoaded(AudioPlaylist playlist)
        {
            builder.setColor(event.getSelfMember().getColor())
                    .setText(FormatUtil.filter(event.getClient().getSuccess()+" 에 대한 검색 결과 `"+event.getArgs()+"`:"))
                    ...
                    .setSelection((msg,i) -> 
                    {
                        ...
                        if(bot.getConfig().isTooLong(track))
                        {
                            event.replyWarning("이 재생목록은 (**"+track.getInfo().title+"**) 허용 된 최대 값보다 깁니다.: `"
        ...
        public void loadFailed(FriendlyException throwable) 
        {
            if(throwable.severity==Severity.COMMON)
                m.editMessage(event.getClient().getError()+" 로딩 오류 : "+throwable.getMessage()).queue();
            else
                m.editMessage(event.getClient().getError()+" 트랙을로드하는 중 오류가 발생했습니다.").queue();
        }
    }
}
