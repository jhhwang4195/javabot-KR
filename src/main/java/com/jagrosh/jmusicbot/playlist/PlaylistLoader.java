                ...
                manager.loadItemOrdered(name, items.get(i), new AudioLoadResultHandler() 
                {
                    ...
                    public void trackLoaded(AudioTrack at) 
                    {
                        if(config.isTooLong(at))
                            errors.add(new PlaylistLoadError(index, items.get(index), "이 트랙이 허용 된 최대 값보다 깁니다."));
                        else
                        {
                            at.setUserData(0L);
                            tracks.add(at);
                    ...
                    {
                        errors.add(new PlaylistLoadError(index, items.get(index), "검색 결과가 없습니다."));
                        done();
                    }
                    ...
                    {
                        errors.add(new PlaylistLoadError(index, items.get(index), "트랙을로드하지 못했습니다.: "+fe.getLocalizedMessage()));
                        done();
                    }
                });
            }
