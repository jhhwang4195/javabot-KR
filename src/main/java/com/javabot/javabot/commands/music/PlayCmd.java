    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist)
        {
            ...
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" 추가되었습니다. **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0?"재생를 시작했습니다.":" 위치에 큐에 "+pos));
            ...
            else
            {
                ...
                if(count==0)
                {
                    m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" 이 재생 목록의 모든 항목 "+(playlist.getName()==null ? "" : "(**"+playlist.getName()
                            +"**) ")+"허용 된 최대 값보다 길었습니다. (`"+bot.getConfig().getMaxTime()+"`)")).queue();
                }
                else
                {
                    m.editMessage(FormatUtil.filter(event.getClient().getSuccess()+" 음악을 찾았습니다. "
                            +(playlist.getName()==null?"에 재생목록":"재생목록 **"+playlist.getName()+"**")+" 와 `"
                            + playlist.getTracks().size()+"`항목; 대기열에 추가되었습니다!"
                            + (count<playlist.getTracks().size() ? "\n"+event.getClient().getWarning()+" 허용 된 최대 값보다 긴 트랙입니다. (`"
                            + bot.getConfig().getMaxTime()+"`) 생략되었습니다." : ""))).queue();
                }
            }
        }
            ...
            event.getChannel().sendMessage(loadingEmoji+" 재생목록을 불려옴니다. **"+event.getArgs()+"**... ("+playlist.getItems().size()+" 아이템)").queue(m -> 
            {
                ...
                playlist.loadTracks(bot.getPlayerManager(), (at)->handler.addTrack(new QueuedTrack(at, event.getAuthor())), () -> {
                    StringBuilder builder = new StringBuilder(playlist.getTracks().isEmpty() 
                            ? event.getClient().getWarning()+" 트랙이로드되지 않았습니다!" 
                            : event.getClient().getSuccess()+" 로드되었 습니다. **"+playlist.getTracks().size()+"** 트렉들!");
